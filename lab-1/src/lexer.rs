...
impl<'a> Lexer<'a> {
    ...
    fn process_alphabetic(&mut self) -> Token {
        ...
        if token::Keyword::is_keyword(&s) {
            match s.as_str() {
                "program" => Token::Keywords(Keyword::Program),
                "begin" => Token::Keywords(Keyword::Begin),
                "end" => Token::Keywords(Keyword::End),
                "var" => Token::Keywords(Keyword::Var),
                "integer" => Token::Keywords(Keyword::Integer),
                "if" => Token::Keywords(Keyword::If),
                "then" => Token::Keywords(Keyword::Then),
                "else" => Token::Keywords(Keyword::Else),
                "do" => Token::Keywords(Keyword::Do),
...
mod tests {
    ...
    fn test_get_token_keywords() {
        ...
        assert_eq!(lexer.get_token(), Ok(Token::Keywords(token::Keyword::Program)));

        let mut lexer = Lexer::new("begin");
        assert_eq!(lexer.get_token(), Ok(Token::Keywords(token::Keyword::Begin)));
        
        let mut lexer = Lexer::new("end");
        assert_eq!(lexer.get_token(), Ok(Token::Keywords(token::Keyword::End)));

        let mut lexer = Lexer::new("var");
        assert_eq!(lexer.get_token(), Ok(Token::Keywords(token::Keyword::Var)));

        let mut lexer = Lexer::new("integer");
        assert_eq!(lexer.get_token(), Ok(Token::Keywords(token::Keyword::Integer)));

        let mut lexer = Lexer::new("if");
        assert_eq!(lexer.get_token(), Ok(Token::Keywords(token::Keyword::If)));

        let mut lexer = Lexer::new("then");
        assert_eq!(lexer.get_token(), Ok(Token::Keywords(token::Keyword::Then)));

        let mut lexer = Lexer::new("else");
        assert_eq!(lexer.get_token(), Ok(Token::Keywords(token::Keyword::Else)));

        let mut lexer = Lexer::new("do");
        assert_eq!(lexer.get_token(), Ok(Token::Keywords(token::Keyword::Do)));
